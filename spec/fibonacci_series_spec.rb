...
RSpec.describe FibonacciSeries do
  describe '#result' do
    context 'when input is zero' do
      ...
      it { expect(FibonacciSeries.new(number).result).to eq 0 }
    end

    context 'when input is one' do
      number = 1

      it { expect(FibonacciSeries.new(number).result).to eq 1 }
    end

    context 'when input is 5' do
      number = 5

      it { expect(FibonacciSeries.new(number).result).to eq 5 }
    end
